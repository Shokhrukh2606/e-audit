...
<body>
    ...
    <div class="ru">
        ...
        <p class="bold text-right">Общество С Ограниченной Ответственностью {{ $conclusion->audit_comp_name_ru }}</p>
            <p>
            Государственная регистрация: Свидетельство Министерства Юстиции РУз. №{{ $conclusion->audit_comp_gov_reg_num }} от 
            {{ date('d.m.Y',strtotime($conclusion->audit_comp_gov_reg_date))}}  года. ИНН -{{ $conclusion->audit_comp_inn }}
            , ОКЭД - {{ $conclusion->audit_comp_oked }}. www.e-audit.uz Лицензия МФ РУз. серия АФ №{{ $conclusion->audit_comp_lic }} от {{ date('d.m.Y',strtotime($conclusion->audit_comp_lic_date))}} года на
            осуществление аудиторских проверок всех хозяйствующих субъектов. {{ $conclusion->audit_comp_bank_name }} Р/С: {{ implode(' ', str_split($conclusion->audit_comp_bank_acc, 4)) }} МФО: {{ $conclusion->audit_comp_bank_mfo }}. Директор аудиторской организации: <strong> {{ $conclusion->audit_comp_director_name }}</strong>, Квалификационный сертификат
            аудитора № {{ $conclusion->audit_comp_director_cert_num }} выдано МФ. РУз. от {{ date('d.m.Y',strtotime($conclusion->audit_comp_director_cert_date)) }} года. 
            </p>
            <p>
            Мною, аудитор аудиторской организации: <strong>{{ $conclusion->audit_comp_name }}</strong> {{ $conclusion->auditor_name }} Клификационный сертификат аудитора
            № {{ $conclusion->auditor_cert_number }} выдано МФ.РУз. от {{ date('d.m.Y', strtotime($conclusion->auditor_cert_date)) }} года, согласно Национального стандарта <strong>(НСАД №80) "Отчет аудитора
            по результатам проверки специального вопроса"</strong> хозяйствующего субъекта {{ $conclusion->cust_info->cust_comp_name }} за период с {{getQuarter('quarter_start', $conclusion->cust_info->quarter_start).".".$conclusion->cust_info->year}} по {{getQuarter('quarter_finish', $conclusion->cust_info->quarter_finish).".".$conclusion->cust_info->year}} года, состоящей из –“Бухгалтерский баланс” - Форма № 1; -“Отчет о финансовых
            результатах” - Форма № 2; 
            </p>
            <p>
            ...
            <p>
            Аудит финансовой отчетности ЧП {{ $conclusion->cust_info->cust_comp_name }} за период с {{getQuarter('quarter_start', $conclusion->cust_info->quarter_start).".".$conclusion->cust_info->year}} по {{getQuarter('quarter_finish', $conclusion->cust_info->quarter_finish).".".$conclusion->cust_info->year}} года выявлено ниже следующие:<br>
            <strong>
            ...
            3. Коэффициент рентабельности расходов {{ $conclusion->kpp()['value'] }}% 
            </strong>
            </p>
            <p>
            По нашему мнению, финансовая отчетность хозяйствующего субъекта {{ $conclusion->cust_info->cust_comp_name }} по состоянию на {{getQuarter('quarter_finish', $conclusion->cust_info->quarter_finish).".".$conclusion->cust_info->year}}
            ...
            за период с {{getQuarter('quarter_start', $conclusion->cust_info->quarter_start).".".$conclusion->cust_info->year}} по {{getQuarter('quarter_finish', $conclusion->cust_info->quarter_finish).".".$conclusion->cust_info->year}} года.
            </p>
